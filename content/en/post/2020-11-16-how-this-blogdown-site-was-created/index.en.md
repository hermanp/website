...
Now, let's see how I managed to get to this point and what hindrances slowed me down. This first post serves as a memo to myself and maybe a helping hand for others.
...
I almost throw it out and will do it if it's necessary. The above illustrates how important to keep up with the changes. Sometimes these changes breaks codes and sometimes the dependency hell messes up. This leads to the actual setup process.
...
After the software environment was set up, I followed along the theme docs: [Getting started](https://wowchemy.com/docs/get-started/). I wanted to get rid of most of the widgets (only keep the experience) on my home page. As the docs offer:
...
So after some foul language and reintall I decided to keep all the widget files and set the `active` parameter to `false`. And lo and behold, it was working. (By the way, this picture wasn't saved previously, but rather found with Google image search: 'hugo academic widget' :heavy_plus_sign: Tools :arrow_right: Color :arrow_right: green)

What have I done (mainly by the docs?)

- avatar + jpg
- website icon
- menus (en/hu)
- color themes (https://wowchemy.com/docs/customization/)
- widget spacing
- settings
  - no map
  - no contact widget
  - no comment
- creative commons footnote and translation and the icons (partials)
...
Note: I heard of [forestry.io](https://forestry.io/) which makes it possible to edit the source files in their web service and commit from there. A command line interface free option to manage the site.
...
# Lessons learned
Devise strategies: read the docs, orient oneself, judge the situation, dependencies, possible hindrances and make a choice or just do everything as fast as possible and if problems arise, new iteration starts (sometimes with a reinstall). If youre lucky, you wont get stuck in the swamp.
